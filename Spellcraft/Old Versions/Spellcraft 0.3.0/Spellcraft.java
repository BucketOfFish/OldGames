...
//Menu elements
	Image menu;
	final int MENU=0,LOAD=1,GAME=2,LOSE=3;
	int menuChoice=0,menuColor=0,menuCount=0,loseColor=255;
//Game elements
	Font spellcraft,timeFont;
	FontMetrics metrics,timeMetrics;
	...
	Magic heal,fire,zap,quake,counterspell,sunMoon,unearth,raiseDead,blast,teleport;
//Startup
	public void init() {
		resize(WIDTH,HEIGHT);
...
//Attributes
		String name;
		boolean undead,flies,wizard,poisoned,frozen,unique,friendly;
		Map location;
...
//Settings
		int RecoverCount=0,walkCount=0,attackCount=0,loseCount=0;
...
//Cast
			if(name.equals("Necromancer")) {
				...
				if(activeSpell.name.equals("Raise Dead")&&(activeSpell.maxCharge-activeSpell.chargeCount)>5&&evade==player) {
					activeSpell=unearth.clone();
				}
			...
			for(int count=0;count<caster.location.population.size();count++) {
				...
				if(me==caster&&!selfInjuring);
					...
					if(me.left()<x&&me.right()>x&&me.up()<y&&me.down()>y)
						...
						distance=Math.sqrt((me.x-x)*(me.x-x)+(me.y-y)*(me.y-y));
					if(distance<size)
						caster.location.population.get(count).changeHP((int)(-blast*(1-distance/size)));
				}
			}
		}
